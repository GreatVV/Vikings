...
{
    /// <summary>
    ///     The maximum time a frame can take. Smaller values allow your character to move faster, larger values will tend to
    ///     make lag
    ...
    protected void MoveInXDirection(bool grounded)
    {
        ...
        var characterInputX = characterInput.x;
        ...
        // Calculate Velocity
        var actualDrag = 1 - (currentDrag * frameTime);
        ...
        if (wall.canWallJump && oppositeDirectionTimer > 0.0f)
        {
            ...
            {
                if (wallJumpDirection == RC_Direction.LEFT)
                {
                    characterInputX = 0.5f;
                }
            }
        ...
        if (characterInputX != 0 && (MyParent == null || !MyParent.overrideX) &&
            (!grounded || !IsSwimming || swimming.canRun))
        {
            var walking = false;
            if (jumpCount > 0 || !grounded || (characterInputX > 0 && characterInputX < 1.0f) ||
                (characterInputX < 0 && characterInputX > -1.0f))
            {
                walking = true;
            }
            if (movement.movementStyle != MovementStyle.PHYSICS_LIKE && movement.jumpAtRunSpeed &&
                (jumpCount > 0 || !grounded))
            {
                walking = false;
            }
            ...
            switch (movement.movementStyle)
            {
                case MovementStyle.PHYSICS_LIKE:
                    var newVelocity = velocity.x +
                                      (frameTime * movement.acceleration * characterInputX) *
                                      (IsSwimming ? (1 - swimming.waterResistance.x) : 1);
                    // Climbing sideways
                    ...
                    else
                    {
                        if (walking)
                        {
                            ...
                            if (velocity.x > movement.walkSpeed && characterInputX >= 0.0f ||
                                velocity.x < movement.walkSpeed * -1 && characterInputX <= 0.0f)
                            {
                                ...
                                {
                                    if (velocity.x > movement.walkSpeed * -1 && characterInputX <= 0.0f)
                                    {
                                        velocity.x = movement.walkSpeed * -1 *
                                                     (IsSwimming ? (1 - swimming.waterResistance.x) : 1);
                                    }
                                }
                            }
                ...
                case MovementStyle.DIGITAL:
                    ...
                    else
                    {
                        if (walking)
                        {
                            velocity.x = (characterInput.x > 0 ? 1 : -1) * movement.walkSpeed *
                                         (IsSwimming ? (1 - swimming.waterResistance.x) : 1);
                        }
                        else
                        {
                            velocity.x = (characterInput.x > 0 ? 1 : -1) * movement.runSpeed *
                                         (IsSwimming ? (1 - swimming.waterResistance.x) : 1);
                        }
                    }
                    break;
        ...
        // Apply velocity
        if ((MyParent == null || !MyParent.overrideX) && velocity.x > movement.skinSize ||
            velocity.x * -1 > movement.skinSize)
        {
            if (targetSlope != 0.0f)
            {
                myTransform.Translate(
                                      Quaternion.Euler(0, 0, targetSlope) *
                                      new Vector3(velocity.x * frameTime, 0.0f, 0.0f));
            }
            else
            {
        ...
        for (var i = 0; i < sides.Length; i++)
        {
            ...
            {
                if (ledgeHanging.autoGrab && velocity.y <= 0.0f)
                {
                    additionalDistance = ledgeHanging.autoGrabDistance;
                }
                else
                {
                    if (wall.canWallSlide)
                    {
                        additionalDistance = wall.wallSlideAdditionalDistance;
                    }
                }
                ...
                if (hitsides2D.collider != null)
                {
                    ...
                    {
                        if (sides[i].direction == RC_Direction.RIGHT &&
                            (sides[i] == highestSideColliders[1] || sides[i] == highestSideColliders[0]))
                        {
                            ledgeHangHitCount++;
                        ...
                        {
                            if (sides[i].direction == RC_Direction.LEFT &&
                                (sides[i] == highestSideColliders[1] || sides[i] == highestSideColliders[0]))
                            {
                                ledgeHangHitCount++;
                    ...
                    if ((wall.canWallSlide || wall.canWallJump) && !grounded)
                    {
                        ...
                        {
                            if (hitsides2D.fraction * (sides[i].distance + additionalDistance) <=
                                sides[i].distance + wall.wallSlideAdditionalDistance)
                            {
                                wallSlideCount++;
                    ...
                    if (hitsides2D.fraction * (sides[i].distance + additionalDistance) <=
                        sides[i].distance + movement.skinSize)
                    {
                        var platform = hitsides2D.collider.gameObject.GetComponent<Platform2D>();
                        if (platform != null)
                        {
                ...
                if (hitsides2D.fraction * (sides[i].distance + additionalDistance) <= sides[i].distance)
                {
                    var tmpForceSide =
                        (hitsides2D.normal *
                         (sides[i].distance - hitsides2D.fraction * (sides[i].distance + additionalDistance))).x;
        ...
        if (ledgeHanging.canLedgeHang && ledgeHangHitCount == 1 && !IsSwimming)
        {
            ...
            if (LedgeHangDirection == RC_Direction.LEFT)
            {
                ...
                if (actualLedgeCollider.IsColliding(
                                                    1 << backgroundLayer,
                                                    ledgeHanging.edgeDetectionOffset.x + ledgeHanging.autoGrabDistance,
                                                    ledgeHanging.edgeDetectionOffset.y))
                {
                    stopLedgeHang = true;
                }
                ...
                if (!stopLedgeHang &&
                    ledgeHangCollider.Max().y <
                    myTransform.position.y + actualLedgeCollider.offset.y + ledgeHanging.graspPoint +
                    ledgeHanging.graspLeeway &&
                    ledgeHangCollider.Max().y >
                    myTransform.position.y + actualLedgeCollider.offset.y + ledgeHanging.graspPoint -
                    ledgeHanging.graspLeeway)
                {
                    ...
                    ledgeHangGoalPosition = ledgeHanging.hangOffset +
                                            new Vector3(
                                                ledgeHangCollider.Max().x,
                                                ledgeHangCollider.Max().y,
                                                myTransform.position.z);
                    // Check for parent
                    var platform = ledgeHangCollider.gameObject.GetComponent<Platform2D>();
                    if (platform != null)
                    {
                        var parentPlatform = platform.ParentOnStand(this);
            ...
            else
            {
                if (LedgeHangDirection == RC_Direction.RIGHT)
                {
                    ...
                    if (actualLedgeCollider.IsColliding(
                                                        1 << backgroundLayer,
                                                        ledgeHanging.edgeDetectionOffset.x +
                                                        ledgeHanging.autoGrabDistance,
                                                        ledgeHanging.edgeDetectionOffset.y))
                    {
                        stopLedgeHang = true;
                    }
        ...
        {
            isWallHolding = true;
            var stopWallSlide = true;
            var hit = actualWallCollider.GetCollision(1 << backgroundLayer, wall.edgeDetectionOffset.x);
            ...
            if (actualWallCollider.IsColliding(
                                               1 << backgroundLayer,
                                               wall.edgeDetectionOffset.x,
                                               wall.edgeDetectionOffset.y))
            {
                stopWallSlide = false;
            }
            if (wall.wallJumpTag != null && wall.wallJumpTag != "" && currentWallTag != wall.wallJumpTag)
            {
                stopWallSlide = true;
            }
            if (!stopWallSlide && wall.canWallSlide &&
                ((wallSlideDirection == RC_Direction.RIGHT && characterInput.x > 0) ||
                 (wallSlideDirection == RC_Direction.LEFT && characterInput.x < 0)))
            {
                ...
                if (hit.collider != null)
                {
                    var platform = hit.collider.gameObject.GetComponent<Platform2D>();
        ...
        {
            if (-1 * forceSide > movement.skinSize)
            {
        ...
        if (grounded)
        {
            ...
            {
                if ((velocity.x > maxSpeedForIdle && characterInput.x > 0.1f) ||
                ...
                {
                    if (velocity.x > maxSpeedForIdle || velocity.x < -1 * maxSpeedForIdle)
                    {
    ...
    protected void MoveInYDirection(bool grounded)
    {
        ...
        var cantClimbDown = false;
        var hasHitHead = false;
        var climbCount = 0;
        GroundedFeetCount = 0;
        IsClimbingUpOrDown = false;

        // Create a copy of character input which we can modify;
        var characterInputY = characterInput.y;
        ...
        if (IsSwimming && velocity.y > swimming.maxYSpeed)
        {
            velocity.y = swimming.maxYSpeed;
        }

        // Apply velocity
        if ((MyParent == null || !MyParent.overrideY) &&
            (velocity.y > movement.skinSize || velocity.y * -1 > movement.skinSize))
        {
        ...
        if ((velocity.y <= 0.0f || StartedClimbing) && ledgeDropTimer <= 0.0f)
        {
            ...
            foreach (var feetCollider in feetColliders)
            {
                var hitFeet = new RaycastHit2D();
                var hitLadder = new RaycastHit2D();
                var hitGround = new RaycastHit2D();
                var closest = float.MaxValue;
                var closestLadder = float.MaxValue;

                var feetCollisions =
                    feetCollider.GetAllCollision(
                                                 1 << backgroundLayer |
                                                 (FallThroughTimer <= 0.0f ? 1 << passThroughLayer : 0) |
                                                 (climbing.allowClimbing && FallThroughTimer <= 0.0f
                                                      ? 1 << climableLayer
                                                      : 0),
                                                 slopes.slopeLookAhead);
                // Get closest collision
                foreach (var collision in feetCollisions)
                {
                    ...
                    {
                        if (collision.fraction * (feetCollider.distance + slopes.slopeLookAhead) < closest)
                        {
                            hitFeet = collision;
                ...
                if (hitLadder.collider != null && hitFeet.collider != null &&
                    hitLadder.fraction * (feetCollider.distance + slopes.slopeLookAhead) < closest)
                {
                    // Only assign ground if its a true hit, not a slope look ahead hit
                    if (hitFeet.fraction * (feetCollider.distance + slopes.slopeLookAhead) <= feetCollider.distance &&
                        hitFeet.collider.gameObject.layer != climableLayer)
                    {
                        hitGround = hitFeet;
                        hasHitFeet = true;
                    }
                ...
                var force =
                    (hitFeet.normal *
                     (feetCollider.distance - (hitFeet.fraction * (feetCollider.distance + slopes.slopeLookAhead)))).y;
                // Standing on a something that has an action when you stand on it
                if (hitFeet.collider != null)
                {
                    var platform = hitFeet.collider.gameObject.GetComponent<Platform2D>();
                    if (platform != null &&
                        feetCollider.distance >= hitFeet.fraction * (feetCollider.distance + slopes.slopeLookAhead))
                    {
                    ...
                    // Climbing 
                    if (climbing.allowClimbing && hitLadder.collider != null &&
                        hitLadder.fraction * (feetCollider.distance + slopes.slopeLookAhead) <= feetCollider.distance &&
                        jumpButtonTimer <= 0.0f)
                    {
                        ...
                        climbCount++;
                        if (!(MyParent is TopStepPlatform2D) &&
                            (StartedClimbing ||
                             (climbing.autoStick && hitGround.collider == null &&
                              (dismounting == null || (MyParent != null && dismounting != ((Ladder2D) MyParent).control)) ||
                              characterInputY != 0)))
                        {
                            if (climbCount >= climbing.collidersRequired)
                            {
                                ...
                                if (MyParent == null || !MyParent.overrideY)
                                {
                                    ...
                                    {
                                        if (!cantClimbDown && characterInputY < 0.0f)
                                        {
                                            ...
                                            if (hitGround.collider != null)
                                            {
                                                var groundForce =
                                                    (hitGround.normal *
                                                     (feetCollider.distance -
                                                      hitGround.fraction *
                                                      (feetCollider.distance + slopes.slopeLookAhead))).y;
                        ...
                        if (hitFeet.collider.gameObject.layer != climableLayer)
                        {
                            ...
                            else
                            {
                                slope +=
                                    Mathf.Atan(
                                               (lastHitDistance -
                                                hitFeet.fraction * (feetCollider.distance + slopes.slopeLookAhead)) /
                                               (feetCollider.offset.x - lastHitX)) * Mathf.Rad2Deg;
                                slopeCount++;
                                lastHitDistance = hitFeet.fraction * (feetCollider.distance + slopes.slopeLookAhead);
                                lastHitX = feetCollider.offset.x;
                            }
                    ...
                    if (hitLadder.collider == null && characterInputY < 0.0f &&
                        hitFeet.fraction * (feetCollider.distance + slopes.slopeLookAhead) <= feetCollider.distance)
                    {
                        cantClimbDown = true;
                        maxForce = 0.0f;
                    }
            ...
            if (hasHitFeet)
            {
                ...
                if ((MyParent == null || !MyParent.overrideY))
                {
                    if ((MyParent is LadderCollider2D &&
                         ((LadderCollider2D) MyParent).control.ShouldPlayLedgeClimb(this)))
                    {
                        if (characterInput.y < 0.0f && MyParent is TopStepPlatform2D &&
                            climbCount >= climbing.collidersRequired && GroundedFeetCount == 0)
                        {
                        ...
                        else
                        {
                            if (characterInput.y > 0.0f && !(MyParent is TopStepPlatform2D))
                            {
                                ...
                                myTransform.position = new Vector3(
                                    ...
                                    ((LadderCollider2D) MyParent).LedgeClimbHeight,
                                    myTransform.position.z);
                            }
                            else
                            {
        ...
        else
        {
            ...
            var holdOffOnGravity = false;
            foreach (var foot in feetColliders)
            {
        ...
        {
            var actualSlope = (myTransform.localEulerAngles.z % 360.0f) + (slope / slopeCount);
            if (slopeCount > 0 && !isClimbing &&
                (!(actualSlope > slopes.maxRotation && actualSlope < 360.0f - slopes.maxRotation)))
            {
                myTransform.Rotate(0.0f, 0.0f, slopes.rotationSpeed * (slope / slopeCount));
            }
            else
            {
                if (slopeCount == -1 || isClimbing)
                {
                    myTransform.localRotation = Quaternion.RotateTowards(
                                                                         myTransform.localRotation,
                                                                         Quaternion.identity,
                                                                         slopes.rotationSpeed * 10.0f);
                }
            }
        ...
        {
            var maxForce = 0.0f;
            foreach (var headCollider in headColliders)
            {
                var hitHead = headCollider.GetCollision(1 << backgroundLayer);
                var force = (hitHead.normal * (headCollider.distance - (hitHead.fraction * headCollider.distance))).y;
                if (hitHead.collider != null)
                {
                    // Action on headbut
                    var platform = hitHead.collider.gameObject.GetComponent<Platform2D>();
                    if (platform != null)
                    {
        ...
        if (!hasHitHead || isClimbing)
        {
            if (characterInput.jumpButtonDown)
            {
                ...
                {
                    if (jumpCount == 1 && jump.canDoubleJump && !IsSwimming)
                    {
                        Unparent();
                        StartedClimbing = false;
                        jumpCount++;
            ...
            {
                if (characterInput.jumpButtonHeld && jumpHeldTimer < jump.jumpHeldTime && jumpCount == 1)
                {
        ...
        if (wall.canWallJump && (wall.wallJumpTag == "" || wall.wallJumpTag == currentWallTag))
        {
            ...
            {
                if ((hasPressedJumpForWallJump &&
                     ((wallJumpDirection == RC_Direction.LEFT && characterInput.x > 0) ||
                      (wallJumpDirection == RC_Direction.RIGHT && characterInput.x < 0))) ||
                ...
                {
                    if (!hasPressedJumpForWallJump && characterInput.jumpButtonDown)
                    {
                        hasPressedJumpForWallJump = true;
                    }
                    else
                    {
                        if (!hasPressedDirectionForWallJump &&
                            ((wallJumpDirection == RC_Direction.LEFT && characterInput.x > 0) ||
                             (wallJumpDirection == RC_Direction.RIGHT && characterInput.x < 0)))
                        {
    ...
    public void DoCrouch()
    {
        var hasHitHead = false;
        var inTheAir =
            !(jumpCount == 0 && State != CharacterState.FALLING && State != CharacterState.AIRBORNE &&
              State != CharacterState.JUMPING);
        ...
        if (isCrouchSliding)
        {
            foreach (var headCollider in headColliders)
            {
                ...
                if (crouch.useHeightReduction)
                {
                    additionalDistance = crouch.heightReductionFactor * headCollider.distance;
                }
                else
                {
                    additionalDistance = crouch.headDetectionDistance;
                }
                var hitHead = headCollider.GetCollision(1 << backgroundLayer, additionalDistance);
        ...
        if (isCrouching)
        {
            ...
            if (crouch.canCrouchSlide &&
                (isCrouchSliding &&
                 (velocity.x > crouch.minVelocityForStopSlide || velocity.x < -1 * crouch.minVelocityForStopSlide) ||
        ...
        switch (ledgeHangState)
        {
            case LedgeHangingState.TRANSITION:
                ...
                myTransform.position = Vector3.Slerp(
                                                     ledgeHangOriginalPosition,
                                                     ledgeHangGoalPosition,
                                                     ledgeHangTimer / ledgeHanging.transitionTime);
            ...
            case LedgeHangingState.HANG:
                ...
                {
                    if (characterInput.x < 0.0f && LedgeHangDirection == RC_Direction.RIGHT)
                    {
                        IsLedgeHanging = false;
                        ledgeDropTimer = ledgeHanging.ledgeDropTime;
                        State = CharacterState.FALLING;
            ...
            case LedgeHangingState.FINISHED:
                ...
                else
                {
                    if (LedgeHangDirection == RC_Direction.LEFT)
                    {
                        myTransform.Translate(
                                              new Vector3(
                                                  -1 * ledgeHanging.climbOffset.x,
                                                  ledgeHanging.climbOffset.y,
                                                  ledgeHanging.climbOffset.z));
                    }
                }
                State = CharacterState.IDLE;
                break;
            ...
            if (wall.canWallSlide && isWallSliding)
            {
                ...
                else
                {
                    if (velocity.y > movement.terminalVelocity * wall.wallSlideGravityFactor)
                    {
                        ...
                        if (velocity.y < movement.terminalVelocity * wall.wallSlideGravityFactor)
                        {
                            velocity.y = movement.terminalVelocity * wall.wallSlideGravityFactor;
                        }
                    }
                    else
                    {
    ...
    private bool characterStateForced;
    ...
    public RaycastCharacterController2D()
    {
        IsLedgeHanging = false;
        FallThroughTimer = 0.0f;
        StartedClimbing = false;
        IsClimbingUpOrDown = false;
    }
    ...
    /// </summary>
    public bool IsClimbingUpOrDown { get; private set; }
    ...
    /// </value>
    public bool StartedClimbing { get; private set; }
    ...
    public CharacterState State
    {
        ...
        private set
        {
            if (!characterStateForced && (int) value > (int) potentialState)
            {
                potentialState = value;
            }
        }
    }
    ...
    public bool IsGrounded(float offset, bool includeClimables = true)
    {
        foreach (var foot in feetColliders)
        {
            if (
                foot.IsColliding(
                                 1 << backgroundLayer | 1 << passThroughLayer |
                                 (includeClimables ? 1 << climableLayer : 0),
                                 offset))
            {
                return true;
            }
    ...
    /// </summary>
    public Platform2D MyParent { get; private set; }

    /// <summary>
    ///     Sets the fall through timer (used by passthrough platforms).
    /// </summary>
    public float FallThroughTimer { get; set; }
    ...
    public int CurrentDirection
    {
        get
        {
            if (directionChecker != null)
            {
                return directionChecker.CurrentDirection;
            }
    ...
    public static float FrameTime
    {
        get
        {
            return Mathf.Min(Time.deltaTime, maxFrameTime);
        }
    }
    ...
    /// </summary>
    public int GroundedFeetCount { get; private set; }
    ...
    public void SwitchColliders()
    {
        ...
        // Feet Collider2Ds
        foreach (var c in feetColliders)
        {
            c.offset = new Vector3(c.offset.x * -1, c.offset.y, c.offset.z);
        }
        // Side Collider2Ds
        foreach (var c in sides)
        {
            ...
            {
                if (c.direction == RC_Direction.RIGHT)
                {
                    c.direction = RC_Direction.LEFT;
                }
            }
            var tempCollider = highestSideColliders[0];
    ...
    /// </summary>
    public RC_Direction LedgeHangDirection { get; private set; }
    ...
    private void Awake()
    {
        ...
        if (sides != null)
        {
            float[] highestSideColliderHeight =
            {
                -9999.0f,
                -9999.0f
            };
            ...
            foreach (var c in sides)
            {
                ...
                {
                    if (c.direction == RC_Direction.LEFT && c.offset.y > highestSideColliderHeight[1])
                    {
    ...
    private void LateUpdate()
    {
        ...
        if (controllerActive)
        {
            var grounded = IsGrounded(groundedLookAhead);
            if (grounded)
            {
                fallingTime = 0.0f;
            }
            ...
            {
                if (climbing.allowClimbing && isLadderTopClimbing)
                {
                    DoLadderTop();
                }
                else
                {
                    ...
                    if (stunTimer <= 0.0f || stun == StunType.STOP_INPUT_ONLY ||
                        stun == StunType.STOP_INPUT_AND_Y_MOVEMENT)
                    {
                        MoveInXDirection(grounded);
                    }
                    if (!IsLedgeHanging &&
                        (stunTimer <= 0.0f || stun == StunType.STOP_INPUT_ONLY ||
                         stun == StunType.STOP_INPUT_AND_X_MOVEMENT))
                    {
...
public class RaycastCollider2D
{
    ...
    public bool IsColliding()
    {
        return Physics2D.Raycast(
                                 ...
                                 transform.localRotation * GetVectorForDirection(),
                                 distance);
    }
    ...
    public bool IsColliding(int layerMask, float extraDistance, float yOffset)
    {
        return
            Physics2D.Raycast(
                              transform.position +
                              transform.localRotation * new Vector3(offset.x, offset.y + yOffset, offset.z),
                              transform.localRotation * GetVectorForDirection(),
    ...
    public void DrawRayCast()
    {
        if (transform != null)
        {
            switch (direction)
            {
                case RC_Direction.DOWN:
                    Gizmos.color = Color.green;
                    break;
                case RC_Direction.RIGHT:
                    Gizmos.color = Color.red;
                    break;
                case RC_Direction.LEFT:
                    Gizmos.color = Color.yellow;
                    break;
                case RC_Direction.UP:
                    Gizmos.color = Color.magenta;
                    break;
            }
            var position = transform.position + transform.localRotation * offset;
